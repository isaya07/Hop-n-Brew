<template>
  <div class="item center">
      <div class="item-inline">
        ...
        <span class="select is-flex-grow-4">
        ...
        <button class=" btn last" @click="addIngredient">Add</button>
      </div>
      ...
      <transition name="fade">
        <div v-show="contentVisible" class="item-content grid-2 medium-1 small-1">
          <div v-for="(value, key) in ingredientData" class="item-data col middle grid-2" :class="key === 'notes' ? 'item-last is-full' : ''">
...
export default {
  ...
  methods: {
    ...
    addIngredient () {
      if (this.ingredientData.amount === 0 || this.ingredientData.amount === undefined) {
        this.error.has = true
        this.error.mess = 'Please enter weight value'
      } else if (this.type === 'hop' && (this.ingredientData.time === 0 || this.ingredientData.time === undefined)) {
        this.error.has = true
        this.error.mess = 'Please enter time value'
      } else {
        this.ingredientData.displayAmount = this.ingredientData.amount + ' ' + this.weightUnitie
        if (this.type === 'hop') {
          this.ingredientData.displayTime = this.ingredientData.time + ' ' + this.timeUnitie
        }
        this.$emit('add-ingredient', this.ingredientData)
      }
    }
  }
}
